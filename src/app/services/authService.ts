...
export const authService = {
  ...
  // 2. Fazer Cadastro
  async cadastrar(nome: string, matricula: string, senha: string) {
    const emailFake = `${matricula}${DOMINIO_SISTEMA}`;
    ...
    if (error) {
      if (error.message.includes('User already registered')) {
        throw new Error('Esta matrícula já está registada no sistema.');
      }
  ...
  // 4. Helper para formatar os dados do Supabase para o nosso formato limpo
